...
def seq2esm2_dict_construction(whole_seq, esm2_model, aa2index, seqwin):    
    ...
    with torch.no_grad():
        results = esm2_model(tokens, repr_layers=[5], return_contacts=True)
    token_representations = results["representations"][5] # seqwin+2
...
def emb_seq_prott5(seq, prott5_model, seqwin, features):
    emb_mat = prott5_model[seq][..., :features]
    
    # シーケンス長の確認とパディング
    if emb_mat.shape[0] < seqwin:
        pad_size = seqwin - emb_mat.shape[0]
        pad = torch.zeros(pad_size, features)
        emb_mat = torch.cat([emb_mat, pad], dim=0)
    elif emb_mat.shape[0] > seqwin:
        emb_mat = emb_mat[:seqwin, :]
...
class pv_data_sets():
    def __init__(self, data_sets, encode_method, aa2be_dict, kmer, w2v_model, features, sp, esm2_model, seq2esm2_dict, nn_embed, seqwin,prott5_file):
        super().__init__()
        ...
        self.seqwin = seqwin
        self.prott5 = self.load_prott5_data(prott5_file)

    def load_prott5_data(self, file_path):
        data = {}
        with h5py.File(file_path, 'r') as f:
            for key in f.keys():
                # 各キーに対応するデータセットをロードし、テンソルに変換
                data[key] = torch.tensor(f[key][()])  # 必要に応じてdtypeを指定
        return data
    ...
    def __getitem__(self, idx):
        ...
        if self.encode_method == 'BE':
            ...
            return emb_mat.float().to(device), torch.tensor(label).to(device)
        elif self.encode_method == 'prott5':
            emb_mat = emb_seq_prott5(self.seq[idx], self.prott5, self.seqwin, self.features)
            # print(emb_mat.shape)
            return emb_mat.float().to(device), torch.tensor(label).to(device)
        else:
...
class train_test_process():
    ...
    def training_testing(self, train_data_sets, val_data_sets, test_data_sets, deep_method, encode_method, seqwin, kmer, w2v_model, vector_size, sp_bpe, esm2_model, seq2esm2_dict,prott5_file):
        ...
        tra_data_all = pv_data_sets(train_data_sets, encode_method, aa2be_dict, kmer, w2v_model, features, sp_bpe, esm2_model, seq2esm2_dict, nn_embed, seqwin,prott5_file)
        train_loader = DataLoader(dataset = tra_data_all, batch_size = self.tra_batch_size, shuffle=True)
        # for batch_idx, (data, labels) in enumerate(train_loader):
        #     print(f"Batch {batch_idx + 1}")
        #     print(f"Data shape: {data.shape}")    # [batch_size, seq_length, d_model]
        #     print(f"Labels shape: {labels.shape}") # [batch_size]
        #     break  # 最初のバッチのみ確認


        val_data_all = pv_data_sets(val_data_sets, encode_method, aa2be_dict, kmer, w2v_model, features, sp_bpe, esm2_model, seq2esm2_dict, nn_embed, seqwin,prott5_file)
        val_loader = DataLoader(dataset = val_data_all, batch_size = self.val_batch_size, shuffle=False)
        
        test_data_all = pv_data_sets(test_data_sets, encode_method, aa2be_dict, kmer, w2v_model, features, sp_bpe, esm2_model, seq2esm2_dict, nn_embed, seqwin,prott5_file)
        ...
        if deep_method == 'CNN':
            ...
            if encode_method == 'ESM2':   
                ...
                net = AIMP(pre_feas_dim=1024, hidden=1024, n_transformer=1, dropout=0.5).to(device)
            else :
        ...
        with open(self.out_path + "/cv_result.txt", 'w') as f:
            ...
            for epoch in range(self.n_epoch):
                ...
                for i, (emb_mat, label) in enumerate(train_loader):
                    ...
                    opt.zero_grad()
                    outputs = net(emb_mat)
                    # outputs = outputs.unsqueeze(-1)  # 次元を追加して [128] -> [128, 1] に変更
                    # print(outputs.shape)
                    # print(label.shape)

                    assert outputs.shape == label.shape, f"Shape mismatch in Batch {i}: outputs {outputs.shape}, labels {label.shape}"
                    ...
                    # print(f"Batch {i}: outputs shape {outputs.shape}, label shape {label.shape}")
...
if __name__ == '__main__':
    ...
    prott5_file = parser.parse_args().prott5
    ...
    for i in range(1, kfold+1):
        ...
        net.training_testing(train_dataset, val_dataset, test_dataset, deep_method, encode_method, seqwin, kmer, w2v_model, size, sp_bpe, esm2_model, seq2esm2_dict,prott5_file)
